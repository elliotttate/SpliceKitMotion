//
//  SpliceKitServer.m
//  The brain of SpliceKit — JSON-RPC 2.0 server that listens on the configured
//  TCP localhost port (default 9878 in the MotionKit fork).
//
...
#define SPLICEKIT_DEFAULT_TCP_PORT 9878

static int sServerFd = -1;
static int sConfiguredTCPPort = -1;
static dispatch_source_t sAcceptSource = nil;

static int SpliceKit_tcpPort(void) {
    if (sConfiguredTCPPort > 0) return sConfiguredTCPPort;

    const char *env = getenv("MOTIONKIT_PORT");
    if (env && *env) {
        char *end = NULL;
        long value = strtol(env, &end, 10);
        if (end && *end == '\0' && value > 0 && value <= 65535) {
            sConfiguredTCPPort = (int)value;
            return sConfiguredTCPPort;
        }
    }

    sConfiguredTCPPort = SPLICEKIT_DEFAULT_TCP_PORT;
    return sConfiguredTCPPort;
}
...
static NSString *SpliceKit_safeDescription(id object, NSUInteger limit) {
    if (!object) return @"nil";
    ...
    if (desc.length == 0) {
        ...
        desc = [NSString stringWithFormat:@"<%@:%p>", className, object];
    }

    if (limit > 0 && desc.length > limit) {
        return [desc substringToIndex:limit];
    }
    return desc;
}
...
static id SpliceKit_serializeReturnValue(NSInvocation *invocation, BOOL returnHandle) {
    ...
    if (retType[0] == '@') {
        ...
        NSString *className = NSStringFromClass([retObj class]) ?: @"object";
        NSString *desc = SpliceKit_safeDescription(retObj, returnHandle ? 500 : 2000);
        if (returnHandle) {
            NSString *h = SpliceKit_storeHandle(retObj);
            if (!h) {
                return @{@"error": @"Failed to store handle for return value",
                         @"class": className,
                         @"description": desc};
            }
            return @{@"handle": h, @"class": className, @"description": desc};
        }
        return @{@"result": desc, @"class": className};
    }
...
static NSDictionary *SpliceKit_handleSystemCallMethod(NSDictionary *params) {
    ...
    if (!result) {
        return @{@"error": @"Main-thread method call timed out or returned no result"};
    }
    return result;
}
...
static NSDictionary *SpliceKit_handleSystemVersion(NSDictionary *params) {
    ...
    NSString *hostName = info[@"CFBundleName"] ?: info[@"CFBundleDisplayName"] ?: @"unknown";
    NSString *hostBundleID = info[@"CFBundleIdentifier"] ?: @"unknown";
    NSString *hostVersion = info[@"CFBundleShortVersionString"] ?: @"unknown";
    NSString *hostBuild = info[@"CFBundleVersion"] ?: @"unknown";
    NSMutableDictionary *result = [@{
        ...
        @"pid": @(getpid()),
        @"motion_host": @(SpliceKit_isMotionHost()),
        @"arch": @
#if __arm64__
            "arm64"
#else
    ...
    if (!SpliceKit_isMotionHost()) {
        // Legacy fields for FCP clients
        result[@"fcp_version"] = hostVersion;
        result[@"fcp_build"] = hostBuild;
    }
    return result;
}

static NSDictionary *SpliceKit_handleSystemPing(NSDictionary *params) {
    NSDictionary *info = [[NSBundle mainBundle] infoDictionary];
    NSString *hostName = info[@"CFBundleName"] ?: info[@"CFBundleDisplayName"] ?: @"unknown";
    NSString *hostBundleID = info[@"CFBundleIdentifier"] ?: @"unknown";
    NSMutableDictionary *result = [@{
    ...
    // Report main-thread health so clients can detect when Motion is stuck
    int timeoutCount = SpliceKit_consecutiveMainThreadTimeouts();
    if (timeoutCount > 0) {
        result[@"main_thread_timeouts"] = @(timeoutCount);
        result[@"main_thread_healthy"] = @NO;
    } else {
        result[@"main_thread_healthy"] = @YES;
    }

    if (SpliceKit_isMotionHost()) {
        NSDictionary *state = [SpliceKitCommandPalette motionBootstrapState];
        if (state.count > 0) {
            [result addEntriesFromDictionary:state];
            NSInteger documentCount = [state[@"document_count"] integerValue];
            result[@"ready"] = @(documentCount > 0);
        }
        result[@"drain_observer_fires"] =
            @(SpliceKit_mainThreadDrainObserverFireCount());
        result[@"drain_blocks_run"] =
            @(SpliceKit_mainThreadDrainBlocksRunCount());
    }

    // Reset the timeout counter on every ping so the system self-recovers.
    // This allows main-thread operations to be retried on the next request.
    if (timeoutCount > 0) {
        SpliceKit_resetMainThreadTimeouts();
        result[@"main_thread_reset"] = @YES;
    }
...
static NSDictionary *SpliceKit_handleCallMethodWithArgs(NSDictionary *params) {
    ...
    __block NSDictionary *result = nil;
    SpliceKit_log(@"[RPC] system.callMethodWithArgs scheduling %@ %@ (classMethod=%d returnHandle=%d args=%lu)",
                  targetName, selectorName, [params[@"classMethod"] boolValue], returnHandle,
                  (unsigned long)args.count);

    SpliceKit_executeOnMainThread(^{
        SpliceKit_log(@"[RPC] system.callMethodWithArgs entered main thread for %@ %@",
        ...
        @try {
            ...
            SpliceKit_log(@"[RPC] system.callMethodWithArgs completed %@ %@ -> %@",
                          targetName, selectorName, result[@"error"] ?: @"ok");
        } @catch (NSException *e) {
            result = @{@"error": [NSString stringWithFormat:@"Exception: %@ - %@",
                        e.name, e.reason]};
            SpliceKit_log(@"[RPC] system.callMethodWithArgs exception %@ %@: %@ - %@",
                          targetName, selectorName, e.name ?: @"NSException", e.reason ?: @"");
        }
    });
...
static Class SpliceKit_findLoadedClassNamed(const char *wantedName) {
    ...
    return objc_lookUpClass(wantedName);
}
...
void SpliceKit_installEffectDragSwizzlesNow(void) {
    static BOOL sLoggedMotionSkip = NO;
    if (SpliceKit_isMotionHost()) {
        if (!sLoggedMotionSkip) {
            SpliceKit_log(@"[EffectDrag] Skipping FCP-only drag swizzles in Motion");
            sLoggedMotionSkip = YES;
        }
        return;
    }
...
void SpliceKit_installEffectDragAsAdjustmentClip(void) {
    if (SpliceKit_isMotionHost()) {
        SpliceKit_log(@"[EffectDrag] Motion host detected; adjustment-clip drag support disabled");
        return;
    }
    if (!SpliceKit_isEffectDragAsAdjustmentClipEnabled()) {
...
static NSDictionary *SpliceKit_waitForCommandPaletteVisibility(BOOL visible, NSTimeInterval timeout) {
    NSDate *deadline = [NSDate dateWithTimeIntervalSinceNow:timeout];
    while ([deadline timeIntervalSinceNow] > 0) {
        NSDictionary *state = [SpliceKitCommandPalette commandPaletteState];
        if ([state[@"visible"] boolValue] == visible) {
    ...
    NSMutableDictionary *result = [[SpliceKitCommandPalette commandPaletteState] mutableCopy];
    result[@"error"] = visible ? @"Command palette did not open in time" : @"Command palette did not close in time";
    return result;
}

static void SpliceKit_forceActivateHostApplication(void) {
    ...
    OSErr transformError = TransformProcessType(&psn, kProcessTransformToForegroundApplication);
    if (transformError != noErr && transformError != paramErr) {
        SpliceKit_log(@"[RPC] TransformProcessType failed: %d", (int)transformError);
    }

    OSErr frontError = SetFrontProcessWithOptions(&psn, kSetFrontProcessFrontWindowOnly);
    if (frontError != noErr) {
        SpliceKit_log(@"[RPC] SetFrontProcessWithOptions failed: %d", (int)frontError);
    }
}

static void SpliceKit_waitForHostActivation(NSTimeInterval timeout) {
    NSDate *deadline = [NSDate dateWithTimeIntervalSinceNow:timeout];
    while ([deadline timeIntervalSinceNow] > 0) {
        NSRunningApplication *currentApp = [NSRunningApplication currentApplication];
        if (currentApp.active) {
            SpliceKit_log(@"[RPC] Host activation confirmed");
            return;
        }
        SpliceKit_forceActivateHostApplication();
        [NSThread sleepForTimeInterval:0.05];
    }

    NSRunningApplication *currentApp = [NSRunningApplication currentApplication];
    SpliceKit_log(@"[RPC] Host activation timed out (active=%d)", currentApp.active);
}

static BOOL SpliceKit_postCommandPaletteHotkeyEvent(void) {
    NSApplication *app = NSApp ?: [NSApplication sharedApplication];
    if (!app) return NO;

    NSWindow *targetWindow = app.keyWindow ?: app.mainWindow;
    if (!targetWindow) {
    ...
    NSInteger windowNumber = targetWindow ? targetWindow.windowNumber : 0;
    ...
    NSEvent *keyDown = [NSEvent keyEventWithType:NSEventTypeKeyDown
                     ...
                     charactersIgnoringModifiers:@"p"
                                       isARepeat:NO
                                         keyCode:35];
    if (!keyDown) {
        SpliceKit_log(@"[RPC] Failed to create synthetic Cmd+Shift+P keyDown event");
        return NO;
    }
    ...
    NSEvent *keyUp = [NSEvent keyEventWithType:NSEventTypeKeyUp
                   ...
                   charactersIgnoringModifiers:@"p"
                                     isARepeat:NO
                                       keyCode:35];
    [app postEvent:keyDown atStart:NO];
    if (keyUp) {
        [app postEvent:keyUp atStart:NO];
    }

    SpliceKit_log(@"[RPC] Posted synthetic Cmd+Shift+P (window=%ld active=%d)",
                  (long)windowNumber, app.isActive);
    return YES;
}

static NSDictionary *SpliceKit_handleCommandShow(NSDictionary *params) {
    SpliceKit_log(@"[RPC] command.show requested");
    NSDictionary *initialState = [SpliceKitCommandPalette commandPaletteState];
    if ([initialState[@"visible"] boolValue]) {
        NSMutableDictionary *result = [initialState mutableCopy];
        result[@"status"] = @"ok";
        return result;
    }

    if (SpliceKit_isMotionHost()) {
        SpliceKit_waitForHostActivation(1.0);
        BOOL didShow = SpliceKit_executeCocoaUIBlock(^{
            SpliceKitCommandPalette *palette = [SpliceKitCommandPalette sharedPalette];
            if (![palette isVisible]) {
                [palette showPalette];
            }
        }, 1.5, @"rpc.palette.show");
        if (!didShow && !SpliceKit_postCommandPaletteHotkeyEvent()) {
            SpliceKit_requestPaletteAction(1);
        }
        return SpliceKit_waitForCommandPaletteVisibility(YES, 5.0);
    }
    ...
    SpliceKit_executeOnMainThread(^{
        ...
        state = [[SpliceKitCommandPalette commandPaletteState] copy];
    });
    if (!didRun) return @{@"error": @"Command palette show timed out on the main thread"};
    return state ?: SpliceKit_waitForCommandPaletteVisibility(YES, 1.0);
}

static NSDictionary *SpliceKit_handleCommandHide(NSDictionary *params) {
    SpliceKit_log(@"[RPC] command.hide requested");
    NSDictionary *initialState = [SpliceKitCommandPalette commandPaletteState];
    if (![initialState[@"visible"] boolValue]) {
        NSMutableDictionary *result = [initialState mutableCopy];
        result[@"status"] = @"ok";
    ...
    if (SpliceKit_isMotionHost()) {
        SpliceKit_waitForHostActivation(1.0);
        BOOL didHide = SpliceKit_executeCocoaUIBlock(^{
            SpliceKitCommandPalette *palette = [SpliceKitCommandPalette sharedPalette];
            if ([palette isVisible]) {
                [palette hidePalette];
            }
        }, 1.5, @"rpc.palette.hide");
        if (!didHide && !SpliceKit_postCommandPaletteHotkeyEvent()) {
            SpliceKit_requestPaletteAction(2);
        }
        return SpliceKit_waitForCommandPaletteVisibility(NO, 5.0);
    }
    ...
    if (!didRun) return @{@"error": @"Command palette hide timed out on the main thread"};
    return state ?: SpliceKit_waitForCommandPaletteVisibility(NO, 1.0);
}

static NSDictionary *SpliceKit_handleCommandStatus(NSDictionary *params) {
    return [SpliceKitCommandPalette commandPaletteState];
}
...
static NSDictionary *SpliceKit_handleCommandSearch(NSDictionary *params) {
    ...
    for (NSUInteger i = 0; i < MIN(results.count, limit); i++) {
        ...
        [items addObject:@{
            ...
            @"selector": cmd.bridgeSelector ?: [NSNull null],
            @"tag": cmd.bridgeTag ?: [NSNull null],
            @"score": @(cmd.score),
        }];
    }
...
static NSDictionary *SpliceKit_handleMenuExecute(NSDictionary *params) {
    ...
    return result ?: @{@"error": @"Menu execute requires main thread access, which is "
                                 @"unavailable while Motion is rendering. "
                                 @"Use command.execute with motion_command type instead."};
}
...
static NSDictionary *SpliceKit_handleMenuList(NSDictionary *params) {
    ...
    return result ?: @{@"error": @"Menu list requires main thread access, which is "
                                 @"unavailable while Motion is rendering. "
                                 @"Use command.search to find available commands instead."};
}
...
static NSDictionary *SpliceKit_handleDialogDetect(NSDictionary *params) {
    ...
    return result ?: @{@"error": @"Dialog detect requires main thread access, which is "
                                 @"unavailable while Motion is rendering."};
}
...
void SpliceKit_installCrashHandlerNow(void) {
    if (sCrashHandlerInstalled) return;
    NSDictionary *result = SpliceKit_handleDebugCrashHandler(@{@"action": @"install"});
    NSString *message = result[@"message"];
    if (message.length > 0) {
        SpliceKit_log(@"[CRASH] %@", message);
    }
}
...
static NSDictionary *SpliceKit_handleDebugEval(NSDictionary *params) {
    ...
    if (chain && chain.count > 0) {
        ...
        SpliceKit_executeOnMainThread(^{
            @try {
                ...
                res[@"steps"] = steps;
                if (obj) {
                    res[@"result"] = SpliceKit_safeDescription(obj, 2000);
                    res[@"resultClass"] = NSStringFromClass([obj class]) ?: @"object";
                    if (storeResult) {
                        NSString *handle = SpliceKit_storeHandle(obj);
    ...
    SpliceKit_executeOnMainThread(^{
        @try {
            ...
            if (obj) {
                res[@"result"] = SpliceKit_safeDescription(obj, 2000);
                res[@"class"] = NSStringFromClass([obj class]) ?: @"object";
                if (storeResult) {
                    NSString *handle = SpliceKit_storeHandle(obj);
                    if (handle) res[@"handle"] = handle;
                }
...
NSDictionary *SpliceKit_handleRequest(NSDictionary *request) {
    ...
    // --- Motion guard: reject FCP-only methods early ---
    // These methods depend on FCP's FFAnchoredTimelineModule, FFPlayer, Flexo
    // classes, etc. that don't exist in Motion. Without this guard they dispatch
    // to the main thread, wait 20s for a timeout, and return an error anyway —
    // meanwhile Motion is unresponsive.
    if (SpliceKit_isMotionHost()) {
        static NSSet<NSString *> *sFCPOnlyMethods = nil;
        static NSSet<NSString *> *sFCPOnlyPrefixes = nil;
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            sFCPOnlyPrefixes = [NSSet setWithArray:@[
                @"timeline.",
                @"playback.",
                @"fcpxml.",
                @"transitions.",
            ...
            sFCPOnlyMethods = [NSSet setWithArray:@[
                @"inspector.get",
                @"inspector.set",
                @"inspector.getTitle",
                @"view.toggle",
        ...
        BOOL isFCPOnly = [sFCPOnlyMethods containsObject:method];
        if (!isFCPOnly) {
            for (NSString *prefix in sFCPOnlyPrefixes) {
        ...
        if (isFCPOnly) {
            return @{@"error": @{@"code": @(-32601),
                @"message": [NSString stringWithFormat:
                    @"Method \"%@\" is not available in Motion. "
                    @"This method requires Final Cut Pro's timeline engine.", method]}};
        }
    }

    BOOL shouldRunUIPreflight = YES;
    if (SpliceKit_isMotionHost()) {
        // Motion does not use the FCP-only dialog/drag preflight, and forcing a
        // main-thread round trip here can stall simple startup requests such as
        // system.version while Motion is still bringing up its UI.
        shouldRunUIPreflight = NO;
    } else if ([method hasPrefix:@"system."] ||
               [method hasPrefix:@"object."] ||
               [method isEqualToString:@"command.search"]) {
        // Introspection and registry reads do not require timeline/dialog state.
        shouldRunUIPreflight = NO;
    }

    if (shouldRunUIPreflight) {
        SpliceKit_installEffectDragSwizzlesNow();
        // Auto-dismiss known blocking dialogs before processing UI-affecting requests.
    ...
    // system.* namespace
    if ([method isEqualToString:@"system.ping"]) {
        result = SpliceKit_handleSystemPing(params);
    } else if ([method isEqualToString:@"system.version"]) {
    ...
    } else if ([method isEqualToString:@"command.status"]) {
        result = SpliceKit_handleCommandStatus(params);
...
static void SpliceKit_handleClient(int clientFd) {
    ...
    FILE *reader = fdopen(clientFd, "r");
    if (!reader) {
        close(clientFd);
        return;
    }
    int writerFd = dup(clientFd);
    if (writerFd < 0) {
        fclose(reader);
        return;
    }
    FILE *writer = fdopen(writerFd, "w");
    if (!writer) {
        close(writerFd);
        fclose(reader);
        return;
        ...
        @autoreleasepool {
            ...
            NSData *responseJson = [NSJSONSerialization dataWithJSONObject:response
                                                                  options:0
                                                                    error:nil];
            if (responseJson) {
                fwrite(responseJson.bytes, 1, responseJson.length, writer);
                fwrite("\n", 1, 1, writer);
                fflush(writer);
            }
        }
...
#pragma mark - Server
//
// Sets up the TCP listener on 127.0.0.1:9877. We use TCP instead of a Unix
...
void SpliceKit_startControlServer(void) {
    if (sAcceptSource != nil || sServerFd >= 0) {
        SpliceKit_log(@"Control server already running on 127.0.0.1:%d", SpliceKit_tcpPort());
        return;
    }
    ...
    int port = SpliceKit_tcpPort();
    addr.sin_port = htons(port);

    if (bind(serverFd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        SpliceKit_log(@"ERROR: Failed to bind TCP port %d: %s", port, strerror(errno));
        close(serverFd);
        return;
    }
    ...
    SpliceKit_log(@"================================================");
    SpliceKit_log(@"Control server listening on 127.0.0.1:%d", port);
    SpliceKit_log(@"================================================");
    ...
    sAcceptSource = dispatch_source_create(
    ...
    [[NSNotificationCenter defaultCenter]
        addObserverForName:NSApplicationWillTerminateNotification
            ...
            if (sAcceptSource) {
                dispatch_source_cancel(sAcceptSource);
            }
        }];
}
