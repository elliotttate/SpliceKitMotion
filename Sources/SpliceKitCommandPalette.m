...
static id SpliceKitMotionDocumentController(void) {
    Class controllerClass = objc_getClass("MGDocumentController");
    if (!controllerClass) controllerClass = objc_getClass("NSDocumentController");
    if (!controllerClass) return nil;

    for (NSString *selectorName in @[@"sharedDocumentController", @"sharedInstance",
                                     @"shared", @"defaultManager"]) {
        SEL selector = NSSelectorFromString(selectorName);
        if ([controllerClass respondsToSelector:selector]) {
            return ((id (*)(id, SEL))objc_msgSend)((id)controllerClass, selector);
        }
    }
    return nil;
}

static NSArray *SpliceKitMotionDocuments(id controller) {
    if (!controller || ![controller respondsToSelector:@selector(documents)]) return @[];
    id documents = ((id (*)(id, SEL))objc_msgSend)(controller, @selector(documents));
    if ([documents isKindOfClass:[NSArray class]]) return documents;
    return documents ? @[documents] : @[];
}

static void SpliceKitMotionPrepareDocumentWindows(id document) {
    if (!document) return;
    if ([document respondsToSelector:@selector(makeWindowControllers)]) {
        ((void (*)(id, SEL))objc_msgSend)(document, @selector(makeWindowControllers));
    }
    if ([document respondsToSelector:@selector(showWindows)]) {
        ((void (*)(id, SEL))objc_msgSend)(document, @selector(showWindows));
    }
}

static id SpliceKitMotionCurrentDocument(id controller) {
    if (!controller) return nil;
    if ([controller respondsToSelector:@selector(currentDocument)]) {
        id current = ((id (*)(id, SEL))objc_msgSend)(controller, @selector(currentDocument));
        if (current) return current;
    }
    return SpliceKitMotionDocuments(controller).firstObject;
}

static NSDictionary *SpliceKitMotionStateSnapshot(void) {
    id app = NSApp ?: ((id (*)(id, SEL))objc_msgSend)(objc_getClass("NSApplication"),
                                                      @selector(sharedApplication));
    id controller = SpliceKitMotionDocumentController();
    NSArray *documents = SpliceKitMotionDocuments(controller);
    id currentDocument = SpliceKitMotionCurrentDocument(controller);
    NSArray *windows = [app respondsToSelector:@selector(windows)]
        ? ((id (*)(id, SEL))objc_msgSend)(app, @selector(windows))
        : @[];
    ...
    NSMutableArray<NSString *> *windowTitles = [NSMutableArray array];
    for (id window in windows) {
        BOOL visible = [window respondsToSelector:@selector(isVisible)]
            ? ((BOOL (*)(id, SEL))objc_msgSend)(window, @selector(isVisible))
            : NO;
        if (visible) visibleWindowCount++;

        NSString *title = [window respondsToSelector:@selector(title)]
            ? ((id (*)(id, SEL))objc_msgSend)(window, @selector(title))
            : nil;
        if (title.length == 0) title = NSStringFromClass([window class]) ?: @"(untitled)";
        [windowTitles addObject:title];
    }

    NSMutableDictionary *state = [@{
        @"document_controller_class": controller ? (NSStringFromClass([controller class]) ?: @"unknown") : @"",
        @"document_count": @(documents.count),
        @"current_document_class": currentDocument ? (NSStringFromClass([currentDocument class]) ?: @"unknown") : @"",
        @"window_count": @(windows.count),
        @"visible_window_count": @(visibleWindowCount),
        @"window_titles": windowTitles,
        @"app_active": @([app respondsToSelector:@selector(isActive)] ? ((BOOL (*)(id, SEL))objc_msgSend)(app, @selector(isActive)) : NO),
        @"app_hidden": @([app respondsToSelector:@selector(isHidden)] ? ((BOOL (*)(id, SEL))objc_msgSend)(app, @selector(isHidden)) : NO),
        @"finished_launching": @([app respondsToSelector:@selector(isRunning)] ? ((BOOL (*)(id, SEL))objc_msgSend)(app, @selector(isRunning)) : NO),
    } mutableCopy];

    id keyWindow = [app respondsToSelector:@selector(keyWindow)]
        ? ((id (*)(id, SEL))objc_msgSend)(app, @selector(keyWindow))
        : nil;
    NSString *keyWindowTitle = [keyWindow respondsToSelector:@selector(title)]
        ? ((id (*)(id, SEL))objc_msgSend)(keyWindow, @selector(title))
        : nil;
    if (keyWindowTitle.length > 0) {
        state[@"key_window_title"] = keyWindowTitle;
    }

    return state;
}

static NSDictionary *sMotionStateCache = nil;
static BOOL sCommandPaletteBuilt = NO;
static BOOL sCommandPaletteVisible = NO;

static NSDictionary *SpliceKitMotionCachedState(void) {
    @synchronized([SpliceKitCommandPalette class]) {
        return [sMotionStateCache copy];
    }
}

static void SpliceKitMotionUpdateStateCache(NSDictionary *state) {
    if (!state) return;
    @synchronized([SpliceKitCommandPalette class]) {
        sMotionStateCache = [state copy];
    }
}

static void SpliceKitCommandPaletteUpdateState(BOOL built, BOOL visible) {
    @synchronized([SpliceKitCommandPalette class]) {
        sCommandPaletteBuilt = built;
        sCommandPaletteVisible = visible;
    }
}

static NSString *SpliceKitMotionDefaultDocumentType(id controller) {
    if (!controller) return nil;
    SEL defaultTypeSelector = NSSelectorFromString(@"_defaultType:");
    if (![controller respondsToSelector:defaultTypeSelector]) return nil;

    for (NSNumber *flag in @[@YES, @NO]) {
        @try {
            NSString *type = ((id (*)(id, SEL, BOOL))objc_msgSend)(
                controller, defaultTypeSelector, flag.boolValue);
...
static id SpliceKitMotionMakeUntitledDocument(id controller) {
    if (!controller) return nil;

    NSString *documentType = SpliceKitMotionDefaultDocumentType(controller);
    if (documentType.length == 0) {
        SpliceKit_log(@"[Motion] Could not resolve a default untitled document type");
        return nil;
    }

    id document = nil;
    SEL makeWithErrorSelector = NSSelectorFromString(@"makeUntitledDocumentOfType:error:");
    if ([controller respondsToSelector:makeWithErrorSelector]) {
        NSError *error = nil;
        @try {
            document = ((id (*)(id, SEL, id, NSError **))objc_msgSend)(
                controller, makeWithErrorSelector, documentType, &error);
        } @catch (NSException *exception) {
            SpliceKit_log(@"[Motion] makeUntitledDocumentOfType:error: threw %@: %@",
                          exception.name ?: @"NSException",
                          exception.reason ?: @"(no reason)");
        }
        if (!document && error) {
            SpliceKit_log(@"[Motion] makeUntitledDocumentOfType:error: failed: %@",
                          error.localizedDescription ?: @"(unknown)");
        }
    }

    if (!document && [controller respondsToSelector:@selector(makeUntitledDocumentOfType:)]) {
        @try {
            document = ((id (*)(id, SEL, id))objc_msgSend)(
                controller, @selector(makeUntitledDocumentOfType:), documentType);
        } @catch (NSException *exception) {
            SpliceKit_log(@"[Motion] makeUntitledDocumentOfType: threw %@: %@",
                          exception.name ?: @"NSException",
                          exception.reason ?: @"(no reason)");
        }
    }
    ...
    if ([controller respondsToSelector:@selector(addDocument:)]) {
        ((void (*)(id, SEL, id))objc_msgSend)(controller, @selector(addDocument:), document);
    }
    SEL finishSelector = NSSelectorFromString(@"_finishOpeningDocument:andShowWindows:");
    if ([controller respondsToSelector:finishSelector]) {
        ((void (*)(id, SEL, id, BOOL))objc_msgSend)(controller, finishSelector, document, YES);
    } else {
        SpliceKitMotionPrepareDocumentWindows(document);
    }

    id current = SpliceKitMotionCurrentDocument(controller) ?: document;
    if (current) {
        SpliceKit_log(@"[Motion] Bootstrapped document context via makeUntitledDocumentOfType:");
    }
    return current;
}

static id SpliceKitMotionCreateDefaultDocument(id controller) {
    if (!controller) return nil;

    SEL defaultDictionarySelector = NSSelectorFromString(@"dictionaryForDefaultDocument");
    SEL projectBrowserSelector = NSSelectorFromString(@"newDocumentFromProjectBrowser:");
    if (![controller respondsToSelector:defaultDictionarySelector] ||
        ![controller respondsToSelector:projectBrowserSelector]) {
        return nil;
    }

    id payload = nil;
    @try {
        payload = ((id (*)(id, SEL))objc_msgSend)(controller, defaultDictionarySelector);
    } @catch (NSException *exception) {
    ...
    if (!payload) {
        SpliceKit_log(@"[Motion] dictionaryForDefaultDocument returned nil");
        return nil;
    }

    @try {
        ((void (*)(id, SEL, id))objc_msgSend)(controller, projectBrowserSelector, payload);
    } @catch (NSException *exception) {
    ...
    id document = SpliceKitMotionCurrentDocument(controller);
    if (document) {
        SpliceKit_log(@"[Motion] Bootstrapped document context via newDocumentFromProjectBrowser:");
    } else {
        SpliceKit_log(@"[Motion] newDocumentFromProjectBrowser: did not produce a current document");
    }
    return document;
}

static id SpliceKitMotionEnsureDocument(BOOL createIfMissing) {
    id controller = SpliceKitMotionDocumentController();
    if (!controller) {
        SpliceKit_log(@"[Motion] Document bootstrap skipped: no document controller is available yet");
        return nil;
    }

    id document = SpliceKitMotionCurrentDocument(controller);
    if (!document && createIfMissing) {
        document = SpliceKitMotionMakeUntitledDocument(controller);
        if (!document) {
            document = SpliceKitMotionCreateDefaultDocument(controller);
        }

        @try {
            if (!document && [controller respondsToSelector:@selector(newDocument:)]) {
                ((void (*)(id, SEL, id))objc_msgSend)(controller, @selector(newDocument:), nil);
                document = SpliceKitMotionCurrentDocument(controller);
                if (document) {
                    SpliceKit_log(@"[Motion] Bootstrapped document context via newDocument:");
                } else {
                    SpliceKit_log(@"[Motion] newDocument: did not produce a current document");
                }
            }
        } @catch (NSException *exception) {
            SpliceKit_log(@"[Motion] newDocument: threw %@: %@",
                          exception.name ?: @"NSException",
                          exception.reason ?: @"(no reason)");
        }

        if (!document) {
            SEL openUntitled = NSSelectorFromString(@"openUntitledDocumentAndDisplay:error:");
            @try {
                if ([controller respondsToSelector:openUntitled]) {
                    document = ((id (*)(id, SEL, BOOL, id))objc_msgSend)(controller, openUntitled, YES, nil);
                    if (document) {
                        SpliceKit_log(@"[Motion] Bootstrapped untitled document context");
                    } else {
                        SpliceKit_log(@"[Motion] openUntitledDocumentAndDisplay:error: returned nil");
                    }
                }
            } @catch (NSException *exception) {
                SpliceKit_log(@"[Motion] openUntitledDocumentAndDisplay:error: threw %@: %@",
                              exception.name ?: @"NSException",
                              exception.reason ?: @"(no reason)");
            }
        }
        ...
        if (!document &&
            ![controller respondsToSelector:@selector(newDocument:)] &&
            ![controller respondsToSelector:NSSelectorFromString(@"openUntitledDocumentAndDisplay:error:")]) {
            SpliceKit_log(@"[Motion] Document controller %@ has no supported document-creation selector",
...
static id SpliceKitMotionFallbackTargetForSelector(SEL selector, BOOL createIfMissing,
                                                   NSString **source) {
    id controller = SpliceKitMotionDocumentController();
    if ([controller respondsToSelector:selector]) {
        if (source) *source = @"documentController";
        return controller;
    }

    id document = SpliceKitMotionEnsureDocument(createIfMissing);
    if ([document respondsToSelector:selector]) {
        if (source) *source = @"document";
        return document;
    }

    SEL responderSelector = NSSelectorFromString(@"documentKeyResponder");
    if ([document respondsToSelector:responderSelector]) {
        id responder = ((id (*)(id, SEL))objc_msgSend)(document, responderSelector);
        if ([responder respondsToSelector:selector]) {
            if (source) *source = @"documentKeyResponder";
            return responder;
        }
    }
...
static NSString *SpliceKitMotionCanonicalKey(NSString *value) {
    ...
    for (NSUInteger i = 0; i < value.length; i++) {
        ...
        if ([alnum characterIsMember:c]) {
            [result appendFormat:@"%C", (unichar)tolower((int)c)];
        }
    }
    return result;
}
...
static CGFloat FCPFuzzyScore(NSString *query, NSString *target) {
    ...
    NSString *t = [targetString lowercaseString];
    ...
    while (qi < q.length && ti < t.length) {
        ...
        if (qc == tc) {
            ...
            unichar previousOriginal = (ti > 0) ? [targetString characterAtIndex:ti - 1] : 0;
            unichar currentOriginal = [targetString characterAtIndex:ti];
            BOOL separatorBoundary = (ti == 0 ||
                                      [whitespace characterIsMember:previousOriginal] ||
                                      previousOriginal == '/' ||
                                      previousOriginal == '_' ||
                                      previousOriginal == '-');
            BOOL camelBoundary = (ti > 0 &&
                                  [lowercase characterIsMember:previousOriginal] &&
                                  [uppercase characterIsMember:currentOriginal]);
            if (separatorBoundary || camelBoundary) {
                score += 0.3;
            }
            lastMatched = YES;
            qi++;
...
static NSString *SpliceKitCamelCaseToWords(NSString *value) {
    if (value.length == 0) return @"";
    ...
    for (NSUInteger i = 0; i < value.length; i++) {
        ...
        unichar previous = (i > 0) ? [value characterAtIndex:i - 1] : 0;
        unichar next = (i + 1 < value.length) ? [value characterAtIndex:i + 1] : 0;

        BOOL currentUpper = [upper characterIsMember:current];
        BOOL previousLower = (i > 0) && [lower characterIsMember:previous];
        BOOL previousDigit = (i > 0) && [digits characterIsMember:previous];
        BOOL currentDigit = [digits characterIsMember:current];
        BOOL nextLower = (i + 1 < value.length) && [lower characterIsMember:next];
        BOOL previousUpper = (i > 0) && [upper characterIsMember:previous];

        BOOL insertSpace = NO;
        if (i > 0) {
            if ((currentUpper && previousLower) ||
        ...
        if (insertSpace) [result appendString:@" "];
...
static NSString *SpliceKitMotionDisplayName(NSString *commandID) {
    if (commandID.length == 0) return @"";

    NSArray<NSString *> *parts = [commandID componentsSeparatedByString:@"/"];
    NSMutableArray<NSString *> *displayParts = [NSMutableArray arrayWithCapacity:parts.count];
    for (NSString *part in parts) {
        NSString *spaced = SpliceKitCamelCaseToWords(part);
        spaced = [spaced stringByReplacingOccurrencesOfString:@"&" withString:@" and "];
        spaced = [spaced stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        if ([spaced isEqualToString:@"Playfrom Start"]) spaced = @"Play From Start";
        if (spaced.length > 0) [displayParts addObject:spaced];
    }
    return [displayParts componentsJoinedByString:@" / "];
}

static NSArray<NSString *> *SpliceKitMotionKeywords(NSString *commandID,
                                                    NSString *selectorName,
                                                    NSArray<NSString *> *groups) {
    NSMutableOrderedSet<NSString *> *keywords = [NSMutableOrderedSet orderedSet];
    if (commandID.length > 0) {
        [keywords addObject:commandID];
        NSString *displayName = SpliceKitMotionDisplayName(commandID);
        if (displayName.length > 0) [keywords addObject:displayName];
        for (NSString *part in [commandID componentsSeparatedByString:@"/"]) {
            NSString *spaced = SpliceKitMotionDisplayName(part);
            if (spaced.length > 0) [keywords addObject:spaced];
        }
    }
    if (selectorName.length > 0) {
        [keywords addObject:selectorName];
        NSString *selectorWords = SpliceKitCamelCaseToWords(
            [selectorName stringByReplacingOccurrencesOfString:@":" withString:@""]);
        if (selectorWords.length > 0) [keywords addObject:selectorWords];
    }
    for (NSString *groupName in groups) {
        if (groupName.length > 0) [keywords addObject:groupName];
    }
    return keywords.array;
}

static SpliceKitCommandCategory SpliceKitMotionCategoryForGroup(NSString *groupName) {
    NSString *normalized = groupName.lowercaseString;
    if ([normalized isEqualToString:@"tools"] || [normalized isEqualToString:@"alignment"]) {
        return SpliceKitCommandCategoryEditing;
    }
    if ([normalized isEqualToString:@"transport"] || [normalized isEqualToString:@"goto"]) {
        return SpliceKitCommandCategoryPlayback;
    }
    if ([normalized isEqualToString:@"mark"]) {
        return SpliceKitCommandCategoryMarkers;
    }
    if ([normalized isEqualToString:@"view"]) {
        return SpliceKitCommandCategoryOptions;
    }
    return SpliceKitCommandCategoryOptions;
}
...
+ (instancetype)sharedPalette {
    ...
    SpliceKit_log(@"[Palette] sharedPalette requested");

    void (^ensureInstance)(void) = ^{
        dispatch_once(&onceToken, ^{
            SpliceKit_log(@"[Palette] sharedPalette creating singleton");
            instance = [[self alloc] init];
            SpliceKit_log(@"[Palette] sharedPalette created singleton %@", instance ? @"ok" : @"nil");
        });
    };
    ...
    if ([NSThread isMainThread]) {
        ...
        if (!didRun) {
            SpliceKit_log(@"[Palette] WARNING: main-thread singleton initialization timed out; falling back to current thread");
            ensureInstance();
        }
    }

    SpliceKit_log(@"[Palette] sharedPalette returning %@", instance ? @"instance" : @"nil");
    return instance;
}

+ (NSDictionary *)motionBootstrapState {
    if (!SpliceKit_isMotionHost()) return @{};

    if (![NSThread isMainThread]) {
        NSDictionary *cached = SpliceKitMotionCachedState();
        if (cached.count > 0) {
            return cached;
        }
        return @{
    ...
    NSDictionary *state = [SpliceKitMotionStateSnapshot() copy];
    SpliceKitMotionUpdateStateCache(state);
    return state ?: @{};
}

+ (void)refreshMotionStateCacheFromMainThreadIfReady {
    // Called from the run-loop drain observer (always on main thread). Cheap:
    // SpliceKitMotionStateSnapshot just enumerates NSApp.windows and
    // NSDocumentController.documents.
    if (!SpliceKit_isMotionHost() || ![NSThread isMainThread]) return;
    NSDictionary *state = [SpliceKitMotionStateSnapshot() copy];
    if (state) SpliceKitMotionUpdateStateCache(state);
}

+ (NSDictionary *)commandPaletteState {
    @synchronized([SpliceKitCommandPalette class]) {
        return @{
            @"built": @(sCommandPaletteBuilt),
            @"visible": @(sCommandPaletteVisible),
            @"hotkey": @"Cmd+Shift+P",
        };
    }
}

+ (void)bootstrapMotionDocumentContextIfNeeded {
    if (!SpliceKit_isMotionHost()) return;

    SpliceKit_executeOnMainThread(^{
        NSDictionary *before = SpliceKitMotionStateSnapshot();
        SpliceKitMotionUpdateStateCache(before);
        if ([before[@"document_count"] unsignedIntegerValue] > 0) {
            return;
        }

        SpliceKit_log(@"[Motion] Document bootstrap requested "
                      @"(docs=%@ windows=%@ visible=%@ controller=%@ active=%@)",
                      before[@"document_count"] ?: @0,
                      before[@"window_count"] ?: @0,
                      before[@"visible_window_count"] ?: @0,
                      [before[@"document_controller_class"] length] > 0 ? before[@"document_controller_class"] : @"(none)",
                      before[@"app_active"] ?: @NO);

        id document = SpliceKitMotionEnsureDocument(YES);
        NSDictionary *after = SpliceKitMotionStateSnapshot();
        SpliceKitMotionUpdateStateCache(after);
        if (document || [after[@"document_count"] unsignedIntegerValue] > 0) {
            SpliceKitCommandPalette *palette = [SpliceKitCommandPalette sharedPalette];
            [palette buildPanelIfNeeded];
            NSApplication *app = [NSApplication sharedApplication];
            if (app && !app.isActive) {
                [app activateIgnoringOtherApps:YES];
                after = SpliceKitMotionStateSnapshot();
                SpliceKitMotionUpdateStateCache(after);
                SpliceKit_log(@"[Motion] Activated Motion after bootstrap (active=%@)",
                              after[@"app_active"] ?: @NO);
            }
            SpliceKit_log(@"[Motion] Document bootstrap state "
                          @"(docs=%@ windows=%@ visible=%@ current=%@ active=%@)",
                          after[@"document_count"] ?: @0,
                          after[@"window_count"] ?: @0,
                          after[@"visible_window_count"] ?: @0,
                          [after[@"current_document_class"] length] > 0 ? after[@"current_document_class"] : @"(none)",
                          after[@"app_active"] ?: @NO);
        }
    });
}

static BOOL SpliceKitMotionCommandIsBlocked(NSString *selectorName, NSString *commandID,
                                            NSString **reasonOut) {
    if ([selectorName isEqualToString:@"showPreferences:"]) {
        if (reasonOut) {
            *reasonOut = @"Motion's Preferences window currently crashes in the tab wrapper path when launched through MotionKit.";
        }
        return YES;
    }
    // Motion's untitled-document path puts an empty OZTimelineView into a
    // runaway redraw that pegs the main thread and never recovers — every
    // RPC call after that point times out. Block any selector that would
    ...
    if ([blockedDocumentCreationSelectors containsObject:selectorName]) {
        if (reasonOut) {
            *reasonOut = @"Creating an untitled Motion document via MotionKit "
                         @"freezes Motion (OZTimelineView runaway redraw). Open a "
                         @"project from Motion's launcher dialog instead.";
        }
        return YES;
    }
...
- (SpliceKitCommand *)motionCommandForIdentifier:(NSString *)identifier
    ...
    NSArray<SpliceKitCommand *> *catalog = self.masterCommands ?: self.allCommands;
    NSArray<SpliceKitCommand *> *motionCommands = [catalog
        filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(SpliceKitCommand *cmd, NSDictionary *_) {
            return [cmd.type isEqualToString:@"motion_command"];
        }]];

    NSMutableArray<SpliceKitCommand *> *matches = [NSMutableArray array];
    for (SpliceKitCommand *candidate in motionCommands) {
        if ([candidate.action caseInsensitiveCompare:trimmed] == NSOrderedSame ||
            [candidate.name caseInsensitiveCompare:trimmed] == NSOrderedSame ||
            [candidate.bridgeSelector caseInsensitiveCompare:trimmed] == NSOrderedSame) {
            [matches addObject:candidate];
        }
    }
    if (matches.count == 1) return matches.firstObject;
    if (matches.count > 1) {
        if (ambiguous) *ambiguous = matches;
        return nil;
    }

    NSString *canonical = SpliceKitMotionCanonicalKey(trimmed);
    if (canonical.length > 0) {
        for (SpliceKitCommand *candidate in motionCommands) {
            if ([SpliceKitMotionCanonicalKey(candidate.action) isEqualToString:canonical] ||
                [SpliceKitMotionCanonicalKey(candidate.name) isEqualToString:canonical] ||
                [SpliceKitMotionCanonicalKey(candidate.bridgeSelector) isEqualToString:canonical]) {
                [matches addObject:candidate];
            }
        }
    }
    ...
    NSArray<SpliceKitCommand *> *ranked = [[self searchCommands:trimmed]
        filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(SpliceKitCommand *cmd, NSDictionary *_) {
            return [cmd.type isEqualToString:@"motion_command"];
        }]];
    if (ranked.count == 0) return nil;
    if (ranked.count == 1) return ranked.firstObject;

    CGFloat firstScore = ranked[0].score;
    CGFloat secondScore = ranked[1].score;
    if (firstScore >= 0.95 || (firstScore - secondScore) >= 0.18) {
        return ranked.firstObject;
    }

    if (ambiguous) {
        *ambiguous = [ranked subarrayWithRange:NSMakeRange(0, MIN((NSUInteger)5, ranked.count))];
    }
    return nil;
}
...
- (NSDictionary *)executeMotionCommandWithAction:(NSString *)action {
    NSArray<SpliceKitCommand *> *ambiguous = nil;
    SpliceKitCommand *matchedCommand = [self motionCommandForIdentifier:action ambiguous:&ambiguous];
    if (!matchedCommand) {
        if (ambiguous.count > 0) {
            NSMutableArray *candidates = [NSMutableArray arrayWithCapacity:ambiguous.count];
            for (SpliceKitCommand *candidate in ambiguous) {
                [candidates addObject:@{
                    @"name": candidate.name ?: @"",
                    @"action": candidate.action ?: @"",
                    @"selector": candidate.bridgeSelector ?: @"",
                    @"score": @(candidate.score),
                }];
            }
            return @{
                @"error": [NSString stringWithFormat:@"Ambiguous Motion command: %@", action ?: @""],
                @"candidates": candidates,
            };
        }
        return @{@"error": [NSString stringWithFormat:@"No Motion command matches %@", action ?: @"<unknown>"]};
    }

    NSString *selectorName = matchedCommand.bridgeSelector ?: action;
    NSNumber *tagNumber = matchedCommand.bridgeTag;
    NSString *commandID = matchedCommand.action ?: action;
    NSString *commandName = matchedCommand.name ?: commandID;
    if (selectorName.length == 0) {
        return @{@"error": [NSString stringWithFormat:@"No selector is registered for %@", action ?: @"<unknown>"]};
    }
    NSString *blockedReason = nil;
    if (SpliceKitMotionCommandIsBlocked(selectorName, commandID, &blockedReason)) {
        return @{
            @"error": blockedReason ?: @"This Motion command is temporarily disabled in MotionKit.",
            @"command": commandID ?: @"",
    ...
    SpliceKit_executeOnMainThreadAsync(^{
        @try {
            ...
            BOOL handled = ((BOOL (*)(id, SEL, SEL, id, id))objc_msgSend)(
                app, @selector(sendAction:to:from:), selector, dispatchTarget, sender);
            if (!handled) {
                id fallbackTarget = nil;
                if ([selectorName isEqualToString:@"newDocument:"]) {
                    // For an explicit "new document" command, route through
                    // NSDocumentController so Motion shows its native template
                    // chooser. Do NOT call SpliceKitMotionEnsureDocument(YES) —
                    // that uses makeUntitledDocumentOfType: which puts an empty
                    // OZTimelineView into a runaway redraw and pegs the main
                    // thread, blocking every subsequent RPC call.
                    fallbackTarget = SpliceKitMotionDocumentController();
                    if (fallbackTarget) fallbackSource = @"documentController";
                } else {
                    // Other commands: use existing responders only. If no
                    // document is open, the command falls through to a clean
                    // "no responder handled" error. Auto-creating an empty
                    // document here would freeze Motion (see comment above).
                    fallbackTarget = SpliceKitMotionFallbackTargetForSelector(selector, NO,
                                                                             &fallbackSource);
                }

                if (fallbackTarget) {
                    ((void (*)(id, SEL, id))objc_msgSend)(fallbackTarget, selector, sender);
                    dispatchTarget = fallbackTarget;
                    handled = YES;
                }
            }
...
- (void)registerMotionCommands {
    ...
    NSString *commandsPath = [[NSBundle mainBundle] pathForResource:@"NSProCommands" ofType:@"plist"];
    NSString *groupsPath = [[NSBundle mainBundle] pathForResource:@"NSProCommandGroups" ofType:@"plist"];
    NSString *additionalPath = [[NSBundle mainBundle] pathForResource:@"NSProCommandsAdditional" ofType:@"plist"];
    NSDictionary *commandsRaw = commandsPath ? [NSDictionary dictionaryWithContentsOfFile:commandsPath] : nil;
    NSDictionary *groupsRaw = groupsPath ? [NSDictionary dictionaryWithContentsOfFile:groupsPath] : nil;
    NSDictionary *additionalRaw = additionalPath ? [NSDictionary dictionaryWithContentsOfFile:additionalPath] : nil;

    if (![commandsRaw isKindOfClass:[NSDictionary class]] || ![groupsRaw isKindOfClass:[NSDictionary class]]) {
        SpliceKit_log(@"[Palette] Motion command metadata missing. Falling back to a minimal command list.");
        NSArray<NSDictionary *> *fallbackCommands = @[
            @{@"id": @"New", @"selector": @"newDocument:", @"group": @"Application"},
            @{@"id": @"Play", @"selector": @"playForwardDummy:", @"group": @"Transport"},
            @{@"id": @"Stop", @"selector": @"stopPlaying:", @"group": @"Transport"},
            @{@"id": @"NextFrame", @"selector": @"nextFrame:", @"group": @"Transport"},
            @{@"id": @"PreviousFrame", @"selector": @"previousFrame:", @"group": @"Transport"},
            @{@"id": @"EditTool", @"selector": @"activateEditTool:", @"group": @"Tools"},
            @{@"id": @"TextTool", @"selector": @"activateTextTool:", @"group": @"Tools"},
        ];
        for (NSDictionary *payload in fallbackCommands) {
            NSString *commandID = payload[@"id"];
            NSString *selectorName = payload[@"selector"];
            NSString *groupName = payload[@"group"] ?: @"Motion";
            SpliceKitCommand *cmd = [[SpliceKitCommand alloc] init];
            cmd.name = SpliceKitMotionDisplayName(commandID);
            cmd.action = commandID;
            cmd.type = @"motion_command";
            cmd.category = SpliceKitMotionCategoryForGroup(groupName);
            cmd.categoryName = groupName;
            cmd.detail = [NSString stringWithFormat:@"Motion selector %@",
                          selectorName];
            cmd.keywords = SpliceKitMotionKeywords(commandID, selectorName, @[groupName]);
            cmd.bridgeSelector = selectorName;
            [cmds addObject:cmd];
        }
    } else {
        NSMutableDictionary<NSString *, NSMutableArray<NSString *> *> *memberships = [NSMutableDictionary dictionary];
        for (NSString *groupName in groupsRaw) {
            NSDictionary *payload = groupsRaw[groupName];
            if (![payload isKindOfClass:[NSDictionary class]]) continue;
            NSArray *commandIDs = payload[@"commands"];
            if (![commandIDs isKindOfClass:[NSArray class]]) continue;
            for (id value in commandIDs) {
                NSString *commandID = [value isKindOfClass:[NSString class]] ? value : [value description];
                if (commandID.length == 0) continue;
                NSMutableArray<NSString *> *groups = memberships[commandID];
                if (!groups) {
                    groups = [NSMutableArray array];
                    memberships[commandID] = groups;
                }
                [groups addObject:groupName];
            }
        }

        NSMutableDictionary<NSString *, NSDictionary *> *mergedCommands = [NSMutableDictionary dictionary];
        for (NSString *commandID in commandsRaw) {
            NSDictionary *payload = commandsRaw[commandID];
            if (![payload isKindOfClass:[NSDictionary class]]) continue;
            mergedCommands[commandID] = payload;
        }
        if ([additionalRaw isKindOfClass:[NSDictionary class]]) {
            for (NSString *commandID in additionalRaw) {
                NSDictionary *payload = additionalRaw[commandID];
                if (![payload isKindOfClass:[NSDictionary class]]) continue;
                NSMutableDictionary *mergedPayload = [NSMutableDictionary dictionary];
                NSDictionary *existingPayload = mergedCommands[commandID];
                if ([existingPayload isKindOfClass:[NSDictionary class]]) {
                    [mergedPayload addEntriesFromDictionary:existingPayload];
                }
                [mergedPayload addEntriesFromDictionary:payload];
                mergedCommands[commandID] = mergedPayload;
            }
        }

        NSArray<NSString *> *sortedCommandIDs = [[mergedCommands allKeys] sortedArrayUsingSelector:@selector(compare:)];
        for (NSString *commandID in sortedCommandIDs) {
            NSDictionary *payload = mergedCommands[commandID];
            if (![payload isKindOfClass:[NSDictionary class]]) continue;
            NSString *selectorName = payload[@"downSelector"];
            if (![selectorName isKindOfClass:[NSString class]] || selectorName.length == 0) continue;
            if (SpliceKitMotionCommandIsBlocked(selectorName, commandID, nil)) continue;

            NSArray<NSString *> *groups = memberships[commandID] ?: @[];
            NSString *categoryName = groups.firstObject;
            if (categoryName.length == 0) {
                NSArray<NSString *> *parts = [commandID componentsSeparatedByString:@"/"];
                categoryName = parts.count > 1 ? parts.firstObject : @"Motion";
            }
            ...
            cmd.action = commandID;
            cmd.type = @"motion_command";
            cmd.category = SpliceKitMotionCategoryForGroup(categoryName);
            cmd.categoryName = categoryName;
            NSNumber *tag = [payload[@"downTag"] isKindOfClass:[NSNumber class]] ? payload[@"downTag"] : nil;
            cmd.bridgeSelector = selectorName;
            cmd.bridgeTag = tag;
            cmd.detail = tag
                ? [NSString stringWithFormat:@"Motion selector %@ (tag %@)", selectorName, tag]
                : [NSString stringWithFormat:@"Motion selector %@", selectorName];
            cmd.keywords = SpliceKitMotionKeywords(commandID, selectorName, groups);
            [cmds addObject:cmd];
        }
    }

    self.masterCommands = cmds;
...
- (void)buildPanelIfNeeded {
    ...
    SpliceKit_log(@"[Palette] buildPanelIfNeeded starting");
    ...
    self.panel = panel;
    SpliceKitCommandPaletteUpdateState(YES, panel.isVisible);

    // Update status
    [self updateStatusLabel];
    SpliceKit_log(@"[Palette] buildPanelIfNeeded finished");
}
...
- (void)showPalette {
    ...
    SpliceKit_log(@"[Palette] showPalette starting");
    [self buildPanelIfNeeded];
    ...
    SpliceKitCommandPaletteUpdateState(YES, self.panel.isVisible);
    ...
    SpliceKit_log(@"[Palette] showPalette finished");
}
...
- (void)hidePalette {
    ...
    SpliceKit_log(@"[Palette] hidePalette starting");
    ...
    SpliceKitCommandPaletteUpdateState(self.panel != nil, self.panel.isVisible);
    SpliceKit_log(@"[Palette] hidePalette finished");
}
...
- (void)windowWillClose:(NSNotification *)notification {
    ...
    SpliceKitCommandPaletteUpdateState(self.panel != nil, NO);
}
...
- (NSDictionary *)executeCommand:(NSString *)action type:(NSString *)type {
    ...
    if ([type isEqualToString:@"motion_command"]) {
        result = [self executeMotionCommandWithAction:action];
...
- (NSString *)buildAgenticSwiftScript:(NSString *)query timelineContext:(NSDictionary *)ctx {
    ...
    return [NSString stringWithFormat:@
        ...
        "    let port = UInt16(ProcessInfo.processInfo.environment[\"MOTIONKIT_PORT\"] ?? \"9878\") ?? 9878\n"
